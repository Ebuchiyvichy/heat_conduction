...
std::vector<double> quasilinear(int n, int t, double h, double tau, int TEST_P, Date my_date)
{
	...
	for (double j = 0; j <= t; j += tau)
	{
											...
											// TEST_P == 3 - два потока на концах
		{
			...
			double mu = (my_date.c * my_date.rho * y1[n - 1] * h / tau + my_date.right_boarder(tau * j, my_date))
			...
			if (TEST_P == 3 )//ИЗ УСЛОВИЙ ПОТОКА, А НЕ ИЗ ПРОГОНКИ???????
			{
				...
				mu = (my_date.c * my_date.rho * my_date.right_boarder(tau * (j - 1), my_date) * h / tau + my_date.left_boarder(tau * j, my_date))
...
std::vector<double> non_linear(int n, int t, double h, double tau, int TEST_P, Date my_date)
{
	...
	for (double j = 0; j <= t; j += tau)
	{
		for (int k = 0; k != M; k++) {
			...
			{
				double kappa = (a[n - 1] / h) / (my_date.c * my_date.rho * h / tau + a[n] / h + a[n - 1] / h);
				double mu = (my_date.c * my_date.rho * y_tmp[n - 1] * h / tau + my_date.right_boarder(tau * j, my_date))
					/ (my_date.c * my_date.rho * h / tau + a[n] / h + a[n - 1] / h);
